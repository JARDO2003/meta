...
async function loadApp() {
  ...
  await Promise.all([
    ...
    loadImmobilisationsFromFirestore(),
  ]);
  updateStats();
  renderPlanComptable();
  initSaisie();
...
// SYSCOHADA UEMOA — Bénin / Côte d'Ivoire
// ══════════════════════════════════════════

let immobilisationsList = [];
let editingImmoId = null;

// ── Durées légales SYSCOHADA UEMOA (art. 45 Acte Uniforme)
const IMMO_DUREES_LEGALES = {
  incorporelle: 3,
  batiment: 20,
  materiel: 5,
  informatique: 3,
...
// ── Comptes SYSCOHADA par catégorie
const IMMO_COMPTES = {
  incorporelle: { actif: '211', amort: '2811' },
  batiment:     { actif: '231', amort: '2831' },
  materiel:     { actif: '2411', amort: '2841' },
  informatique: { actif: '2442', amort: '2844' },
  mobilier:     { actif: '2444', amort: '2844' },
  transport:    { actif: '2451', amort: '2845' },
  autre:        { actif: '248', amort: '2848' },
};

// ── Compte de dotation aux amortissements (6813x)
const COMPTE_DOTATION_AMORT = '6813';

// ── Firestore CRUD
async function loadImmobilisationsFromFirestore() {
  try {
    const col = window._fbCollection(window._db, 'profiles', currentProfile.id, 'immobilisations');
    const q = window._fbQuery(col, window._fbOrderBy('dateAcquisition', 'asc'));
    const snap = await window._fbGetDocs(q);
    immobilisationsList = [];
    snap.forEach((d) => immobilisationsList.push({ ...d.data(), _docId: d.id }));
  } catch (e) {
    console.warn('[COMEO] Immo load:', e.message);
  }
}

async function saveImmoToFirestore(immo) {
  try {
    const col = window._fbCollection(window._db, 'profiles', currentProfile.id, 'immobilisations');
    if (immo._docId) {
      await window._fbSetDoc(window._fbDoc(window._db, 'profiles', currentProfile.id, 'immobilisations', immo._docId), immo, { merge: true });
      return immo._docId;
    } else {
      const ref = await window._fbAddDoc(col, immo);
      return ref.id;
    }
  } catch (e) {
    toast('Erreur sauvegarde immo : ' + e.message, 'error');
    return null;
  }
}

async function deleteImmoFromFirestore(docId) {
  try {
    await window._fbDeleteDoc(window._fbDoc(window._db, 'profiles', currentProfile.id, 'immobilisations', docId));
  } catch (e) {
    toast('Erreur suppression : ' + e.message, 'error');
  }
}
...
function calcAmortissement(immo) {
  const valBrute = Number(immo.valeurBrute) || 0;
  const valResid = Number(immo.valeurResiduelle) || 0;
  const duree = Number(immo.duree) || 5;
  ...
  const baseAmort = valBrute - valResid;
  const tauxLineaire = duree > 0 ? 100 / duree : 20;

  // Prorata première année (mois entamés / 12)
  const moisAcq = dateAcq.getMonth(); // 0-11
  const prorataPremiereAnnee = (12 - moisAcq) / 12;
  ...
  for (let i = 0; i < duree; i++) {
    ...
    if (methode === 'lineaire') {
      if (i === 0) {
        dotation = baseAmort * (tauxLineaire / 100) * prorataPremiereAnnee;
      } else if (i === duree - 1) {
        dotation = vnc - valResid; // solde dernière année
      } else {
        dotation = baseAmort * (tauxLineaire / 100);
      }
    } else {
      // Dégressif : taux majoré SYSCOHADA
      let coeff = duree <= 3 ? 1.25 : duree <= 5 ? 1.75 : 2.25;
      const tauxDegressif = (tauxLineaire * coeff) / 100;
      dotation = (i === 0 ? vnc * tauxDegressif * prorataPremiereAnnee : vnc * tauxDegressif);
      // Basculement linéaire si plus avantageux
      const dotLineaire = (vnc - valResid) / (duree - i);
      if (dotLineaire > dotation) dotation = dotLineaire;
    }

    dotation = Math.max(0, Math.min(dotation, vnc - valResid));
    amortCumul += dotation;
    vnc -= dotation;
    tableau.push({ annee, dotation, amortCumul, vnc: Math.max(vnc, valResid) });
  }

  // VNC & amort cumulé à la date actuelle
  const anneesEcoulees = yr - anneeAcq;
  let amortActuel = 0;
  let vncActuelle = valBrute;
  let dotationExercice = 0;

  tableau.forEach((t) => {
    if (t.annee < yr) {
      amortActuel = t.amortCumul;
      vncActuelle = t.vnc;
    }
    if (t.annee === yr) {
      dotationExercice = t.dotation;
    }
  });
  ...
  return {
    ...
    tauxLineaire,
    amortActuel,
    vncActuelle: Math.max(vncActuelle, valResid),
    dotationExercice,
    prorataPremiereAnnee,
  };
}

// ── Render liste
function renderImmobilisations() {
  const catFilter = document.getElementById('immo-filter-cat')?.value || '';
  const methodeFilter = document.getElementById('immo-filter-methode')?.value || '';
  const search = (document.getElementById('immo-search')?.value || '').toLowerCase();

  let list = immobilisationsList.filter((im) => {
    if (catFilter && im.categorie !== catFilter) return false;
    if (methodeFilter && im.methode !== methodeFilter) return false;
    if (search && !im.designation?.toLowerCase().includes(search) && !im.compte?.includes(search)) return false;
    return true;
  });

  // KPIs
  let totalBrut = 0, totalAmort = 0, totalVNC = 0, totalDot = 0;
  immobilisationsList.forEach((im) => {
    const c = calcAmortissement(im);
    totalBrut += Number(im.valeurBrute) || 0;
    totalAmort += c.amortActuel;
    totalVNC += c.vncActuelle;
    totalDot += c.dotationExercice;
  });
  document.getElementById('immo-kpi-brut').textContent = fn(totalBrut) + ' FCFA';
  document.getElementById('immo-kpi-amort').textContent = fn(totalAmort) + ' FCFA';
  document.getElementById('immo-kpi-vnc').textContent = fn(totalVNC) + ' FCFA';
  document.getElementById('immo-kpi-dotation').textContent = fn(totalDot) + ' FCFA';
  document.getElementById('immo-kpi-nb').textContent = immobilisationsList.length;

  const tbody = document.getElementById('immoBody');
  if (!list.length) {
    tbody.innerHTML = `<tr><td colspan="12"><div class="empty-state"><div class="icon">🏗</div><p>Aucune immobilisation${catFilter || search ? ' (filtre actif)' : ''}</p></div></td></tr>`;
    return;
  }

  const CAT_LABELS = {
    incorporelle: '<span class="badge-cat inc">Incorporelle</span>',
    batiment:     '<span class="badge-cat bi">Bâtiment</span>',
    materiel:     '<span class="badge-cat mat">Matériel</span>',
    mobilier:     '<span class="badge-cat mob">Mobilier</span>',
    transport:    '<span class="badge-cat tr">Transport</span>',
    informatique: '<span class="badge-cat">Informatique</span>',
    autre:        '<span class="badge-cat">Autre</span>',
  };

  tbody.innerHTML = list.map((im) => {
    const c = calcAmortissement(im);
    const pct = im.valeurBrute > 0 ? (c.amortActuel / im.valeurBrute) * 100 : 0;
    const barClass = pct > 80 ? 'low' : pct > 50 ? 'mid' : '';
    const taux = c.tauxLineaire.toFixed(0);
    return `<tr>
      <td><strong>${im.designation}</strong>${im.numInventaire ? `<br><span class="muted" style="font-size:10px">${im.numInventaire}</span>` : ''}</td>
      <td>${CAT_LABELS[im.categorie] || im.categorie}</td>
      <td style="font-family:var(--font-mono);font-size:11px">${im.compte}</td>
      <td class="r">${im.dateAcquisition}</td>
      <td class="r">${fn(im.valeurBrute)}</td>
      <td class="r">${im.duree} ans</td>
      <td class="r" style="font-size:11px">${im.methode === 'degressif' ? 'Dégressif' : 'Linéaire'}</td>
      <td class="r">${fn(c.amortActuel)}</td>
      <td class="r">
        <span class="vnc-bar-wrap"><span class="vnc-bar ${barClass}" style="width:${Math.min(100-pct,100)}%"></span></span>
        <strong>${fn(c.vncActuelle)}</strong>
      </td>
      <td class="r">${taux}%</td>
      <td class="r">${fn(c.dotationExercice)}</td>
      <td>
        <div style="display:flex;gap:4px">
          <button class="btn btn-sm-wire" style="font-size:10px;padding:3px 7px" onclick="openImmoDetail('${im._docId}')">📋</button>
          <button class="btn btn-sm-wire" style="font-size:10px;padding:3px 7px" onclick="editImmo('${im._docId}')">✎</button>
          <button class="btn btn-danger-outline" style="font-size:10px;padding:3px 7px" onclick="deleteImmo('${im._docId}')">✕</button>
        </div>
      </td>
    </tr>`;
  }).join('');
...
// ── Détail tableau annuel
function openImmoDetail(docId) {
  const im = immobilisationsList.find((x) => x._docId === docId);
  if (!im) return;
  const c = calcAmortissement(im);
  const yr = parseInt(document.getElementById('exerciceYear')?.value || new Date().getFullYear());

  document.getElementById('immoDetailTitle').textContent = `📋 ${im.designation} — Tableau d'amortissement`;

  const rows = c.tableau.map((t) => {
    const isCurrent = t.annee === yr;
    return `<tr class="${isCurrent ? 'current-year' : ''}">
      <td>${t.annee}${isCurrent ? ' ← exercice en cours' : ''}</td>
      <td>${fn(t.dotation)}</td>
      <td>${fn(t.amortCumul)}</td>
      <td>${fn(Math.max(t.vnc, 0))}</td>
      <td>${im.valeurBrute > 0 ? ((t.amortCumul/im.valeurBrute)*100).toFixed(1) : 0}%</td>
    </tr>`;
  }).join('');

  document.getElementById('immoDetailContent').innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px">
      <div class="kpi"><div class="kpi-label">Valeur brute</div><div class="kpi-value" style="color:var(--warm);font-size:18px">${fn(im.valeurBrute)} FCFA</div></div>
      <div class="kpi"><div class="kpi-label">VNC au ${yr}</div><div class="kpi-value" style="color:var(--teal);font-size:18px">${fn(c.vncActuelle)} FCFA</div></div>
      <div class="kpi"><div class="kpi-label">Dotation ${yr}</div><div class="kpi-value" style="color:var(--blue);font-size:18px">${fn(c.dotationExercice)} FCFA</div></div>
    </div>
    <div style="font-size:11px;color:var(--muted);margin-bottom:8px">
      Méthode : <strong>${im.methode === 'degressif' ? 'Dégressif' : 'Linéaire'}</strong> · 
      Taux : <strong>${c.tauxLineaire.toFixed(2)}%</strong> · 
      Durée : <strong>${im.duree} ans</strong> · 
      Compte actif : <strong>${im.compte}</strong> · 
      Compte amort. : <strong>${im.compteAmort}</strong>
    </div>
    <table class="dotation-table">
      <thead><tr><th>Exercice</th><th>Dotation</th><th>Amort. cumulé</th><th>VNC</th><th>% amorti</th></tr></thead>
      <tbody>${rows}</tbody>
      <tr class="total-row">
        <td>TOTAL</td>
        <td>${fn(c.tableau.reduce((s,t)=>s+t.dotation,0))}</td>
        <td>—</td><td>${fn(Number(im.valeurResiduelle)||0)}</td><td>100%</td>
      </tr>
    </table>
    <div style="margin-top:16px;display:flex;gap:8px;justify-content:flex-end">
      <button class="btn btn-sm-wire" onclick="genererDotationUnitaire('${im._docId}')">⚡ Générer écriture dotation ${yr}</button>
    </div>
  `;
  document.getElementById('immoDetailModal').style.display = 'flex';
}

function closeImmoDetailModal() {
  document.getElementById('immoDetailModal').style.display = 'none';
}

// ── Modal saisie
function immoAutoCompte() {
  const cat = document.getElementById('immo-cat').value;
  const comptes = IMMO_COMPTES[cat] || IMMO_COMPTES.autre;
  document.getElementById('immo-compte').value = comptes.actif;
  document.getElementById('immo-compte-amort').value = comptes.amort;
  const dureeRec = IMMO_DUREES_LEGALES[cat] || 5;
  document.getElementById('immo-duree').value = String(dureeRec);
  immoCalcPreview();
}

function immoCalcPreview() {
  const valBrute = Number(document.getElementById('immo-valeur-brute')?.value) || 0;
  const valResid = Number(document.getElementById('immo-valeur-residuelle')?.value) || 0;
  const duree = Number(document.getElementById('immo-duree')?.value) || 5;
  const dateAcq = document.getElementById('immo-date-acq')?.value || new Date().toISOString().split('T')[0];

  if (!valBrute) { document.getElementById('immoPreview').style.display = 'none'; return; }

  const tempImmo = {
    valeurBrute: valBrute,
    valeurResiduelle: valResid,
    duree,
    methode: document.getElementById('immo-methode')?.value || 'lineaire',
    dateAcquisition: dateAcq,
  };
  const c = calcAmortissement(tempImmo);
  document.getElementById('prev-taux').textContent = c.tauxLineaire.toFixed(2) + ' %';
  document.getElementById('prev-dot').textContent = fn(c.dotationExercice) + ' FCFA';
  document.getElementById('prev-cum').textContent = fn(c.amortActuel) + ' FCFA';
  document.getElementById('prev-vnc').textContent = fn(c.vncActuelle) + ' FCFA';
  document.getElementById('immoPreview').style.display = 'block';
}

function openImmoModal(id = null) {
  editingImmoId = id;
  document.getElementById('immoModalTitle').textContent = id ? 'Modifier immobilisation' : 'Nouvelle immobilisation';
  if (id) {
    const im = immobilisationsList.find((x) => x._docId === id);
    if (im) {
      document.getElementById('immo-cat').value = im.categorie || 'materiel';
      document.getElementById('immo-compte').value = im.compte || '';
      document.getElementById('immo-compte-amort').value = im.compteAmort || '';
      document.getElementById('immo-designation').value = im.designation || '';
      document.getElementById('immo-num-inv').value = im.numInventaire || '';
      document.getElementById('immo-date-acq').value = im.dateAcquisition || '';
      document.getElementById('immo-valeur-brute').value = im.valeurBrute || '';
      document.getElementById('immo-valeur-residuelle').value = im.valeurResiduelle || 0;
      document.getElementById('immo-fournisseur').value = im.fournisseur || '';
      document.getElementById('immo-methode').value = im.methode || 'lineaire';
      document.getElementById('immo-duree').value = String(im.duree || 5);
      document.getElementById('immo-notes').value = im.notes || '';
      immoCalcPreview();
    }
  } else {
    document.getElementById('immo-cat').value = 'materiel';
    document.getElementById('immo-date-acq').value = new Date().toISOString().split('T')[0];
    document.getElementById('immo-valeur-brute').value = '';
    document.getElementById('immo-valeur-residuelle').value = '0';
    document.getElementById('immo-fournisseur').value = '';
    document.getElementById('immo-notes').value = '';
    document.getElementById('immo-num-inv').value = '';
    document.getElementById('immoPreview').style.display = 'none';
    immoAutoCompte();
  }
  document.getElementById('immoModal').style.display = 'flex';
}

function closeImmoModal() {
  document.getElementById('immoModal').style.display = 'none';
  editingImmoId = null;
}

async function saveImmobilisation() {
  const designation = document.getElementById('immo-designation').value.trim();
  const valeurBrute = Number(document.getElementById('immo-valeur-brute').value);
  const dateAcquisition = document.getElementById('immo-date-acq').value;
  const compte = document.getElementById('immo-compte').value.trim();
  const compteAmort = document.getElementById('immo-compte-amort').value.trim();

  if (!designation || !valeurBrute || !dateAcquisition || !compte) {
    toast('Remplissez les champs obligatoires (*)', 'error'); return;
  }

  const immo = {
    designation,
    categorie: document.getElementById('immo-cat').value,
    compte,
    compteAmort,
    numInventaire: document.getElementById('immo-num-inv').value.trim(),
    dateAcquisition,
    valeurBrute,
    valeurResiduelle: Number(document.getElementById('immo-valeur-residuelle').value) || 0,
    fournisseur: document.getElementById('immo-fournisseur').value.trim(),
    methode: document.getElementById('immo-methode').value,
    duree: Number(document.getElementById('immo-duree').value),
    notes: document.getElementById('immo-notes').value.trim(),
    createdAt: new Date().toISOString(),
  };

  if (editingImmoId) {
    immo._docId = editingImmoId;
    const idx = immobilisationsList.findIndex((x) => x._docId === editingImmoId);
    if (idx !== -1) immobilisationsList[idx] = immo;
  }

  const docId = await saveImmoToFirestore(immo);
  if (!docId) return;

  if (!editingImmoId) {
    immo._docId = docId;
    immobilisationsList.push(immo);
  }

  closeImmoModal();
  renderImmobilisations();
  toast('Immobilisation enregistrée ✓', 'success');
}

function editImmo(docId) {
  openImmoModal(docId);
}

async function deleteImmo(docId) {
  if (!confirm('Supprimer cette immobilisation ?')) return;
  await deleteImmoFromFirestore(docId);
  immobilisationsList = immobilisationsList.filter((x) => x._docId !== docId);
  renderImmobilisations();
  toast('Immobilisation supprimée', 'info');
}

function resetImmoFiltre() {
  document.getElementById('immo-filter-cat').value = '';
  document.getElementById('immo-filter-methode').value = '';
  document.getElementById('immo-search').value = '';
  renderImmobilisations();
}

// ── Génération écriture dotation (unitaire)
async function genererDotationUnitaire(docId) {
  const im = immobilisationsList.find((x) => x._docId === docId);
  if (!im) return;
  const c = calcAmortissement(im);
  const yr = parseInt(document.getElementById('exerciceYear')?.value || new Date().getFullYear());

  if (!c.dotationExercice || c.dotationExercice <= 0) {
    toast('Aucune dotation à générer pour cet exercice', 'info'); return;
  }

  const dateEcriture = `${yr}-12-31`;
  const libelle = `Dot. amort. ${im.designation} — ${yr}`;
  const ecriture = {
    date: dateEcriture,
    journal: 'OD',
    piece: `AMORT-${yr}-${im.numInventaire || im._docId?.substring(0,6)}`,
    libelle,
    lignes: [
      { compte: COMPTE_DOTATION_AMORT, libelle, debit: Math.round(c.dotationExercice), credit: 0 },
      { compte: im.compteAmort, libelle, debit: 0, credit: Math.round(c.dotationExercice) },
    ],
    source: 'immo',
  };

  // Vérifier si déjà générée cette année
  const dejaGen = ecritures.find((e) =>
    e.source === 'immo' && e.piece?.includes(`AMORT-${yr}`) &&
    e.lignes?.some((l) => l.compte === im.compteAmort)
  );
  if (dejaGen) {
    toast(`Écriture dotation déjà générée pour ${yr}`, 'error'); return;
  }

  const docIdEcr = await saveEcritureToFirestore(ecriture);
  if (docIdEcr) {
    ecritures.push({ ...ecriture, _docId: docIdEcr });
    updateStats();
    toast(`✓ Dotation ${fn(c.dotationExercice)} FCFA générée en OD`, 'success');
    closeImmoDetailModal();
  }
}

// ── Génération TOUTES les dotations de l'exercice
async function genererDotationsAnnuelles() {
  const yr = parseInt(document.getElementById('exerciceYear')?.value || new Date().getFullYear());
  if (!immobilisationsList.length) { toast('Aucune immobilisation enregistrée', 'error'); return; }
  if (!confirm(`Générer les écritures de dotation aux amortissements pour l'exercice ${yr} ?`)) return;

  let nbGeneres = 0;
  let totalDot = 0;

  for (const im of immobilisationsList) {
    const c = calcAmortissement(im);
    if (!c.dotationExercice || c.dotationExercice <= 0) continue;

    const dejaGen = ecritures.find((e) =>
      e.source === 'immo' &&
      e.piece?.includes(`AMORT-${yr}`) &&
      e.lignes?.some((l) => l.compte === im.compteAmort && l.credit > 0)
    );
    if (dejaGen) continue;
    ...
    const ecriture = {
      date: `${yr}-12-31`,
      journal: 'OD',
      piece: `AMORT-${yr}-${im.numInventaire || im._docId?.substring(0,6) || nbGeneres}`,
      libelle,
      lignes: [
        { compte: COMPTE_DOTATION_AMORT, libelle, debit: Math.round(c.dotationExercice), credit: 0 },
        { compte: im.compteAmort, libelle, debit: 0, credit: Math.round(c.dotationExercice) },
  ...
  updateStats();
  if (nbGeneres > 0) {
    toast(`✓ ${nbGeneres} écriture(s) générée(s) — ${fn(totalDot)} FCFA de dotations`, 'success');
    renderImmobilisations();
  } else {
    toast(`Toutes les dotations ${yr} sont déjà générées`, 'info');
  }
}

// ── Export PDF tableau amortissement
function exportImmoPDF() {
  const { jsPDF } = window.jspdf;
  ...
  doc.setFontSize(14);
  doc.setTextColor(212, 168, 83);
  doc.text('TABLEAU DES IMMOBILISATIONS ET AMORTISSEMENTS', 148, 16, { align: 'center' });
  doc.setFontSize(9);
  doc.setTextColor(100);
  doc.text(`${company} — Exercice ${yr} — SYSCOHADA UEMOA`, 148, 22, { align: 'center' });

  const rows = immobilisationsList.map((im) => {
    const c = calcAmortissement(im);
    return [
      im.designation,
      im.compte,
      im.dateAcquisition,
      im.methode === 'degressif' ? 'Dégressif' : 'Linéaire',
      `${im.duree} ans`,
      fnPDF(im.valeurBrute),
      fnPDF(c.amortActuel),
      fnPDF(c.vncActuelle),
      fnPDF(c.dotationExercice),
    ];
  });

  doc.autoTable({
    startY: 28,
    head: [['Désignation', 'Compte', 'Date acq.', 'Méthode', 'Durée', 'Valeur brute', 'Amort. cumulé', 'VNC', `Dotation ${yr}`]],
    body: rows,
  ...
  // Totaux
  const totalBrut = immobilisationsList.reduce((s, im) => s + (Number(im.valeurBrute)||0), 0);
  const totalAmort = immobilisationsList.reduce((s, im) => s + calcAmortissement(im).amortActuel, 0);
  const totalVNC = immobilisationsList.reduce((s, im) => s + calcAmortissement(im).vncActuelle, 0);
  const totalDot = immobilisationsList.reduce((s, im) => s + calcAmortissement(im).dotationExercice, 0);
  const finalY = doc.lastAutoTable.finalY + 6;
  doc.setFontSize(9);
  doc.setFont(undefined, 'bold');
  doc.text(`TOTAUX — Brut: ${fnPDF(totalBrut)} | Amort: ${fnPDF(totalAmort)} | VNC: ${fnPDF(totalVNC)} | Dotation ${yr}: ${fnPDF(totalDot)} FCFA`, 14, finalY);

  doc.save(`COMEO_Immobilisations_${company.replace(/\s+/g,'_')}_${yr}.pdf`);
  toast('PDF tableau amortissement exporté ✓', 'success');
}

// ── Exposition globale
window.renderImmobilisations = renderImmobilisations;
window.openImmoModal = openImmoModal;
window.closeImmoModal = closeImmoModal;
window.saveImmobilisation = saveImmobilisation;
window.editImmo = editImmo;
window.deleteImmo = deleteImmo;
window.resetImmoFiltre = resetImmoFiltre;
window.openImmoDetail = openImmoDetail;
window.closeImmoDetailModal = closeImmoDetailModal;
window.genererDotationUnitaire = genererDotationUnitaire;
window.genererDotationsAnnuelles = genererDotationsAnnuelles;
window.exportImmoPDF = exportImmoPDF;
window.immoAutoCompte = immoAutoCompte;
window.immoCalcPreview = immoCalcPreview;
